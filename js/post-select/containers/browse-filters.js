...
const applyWithSelect = withSelect( ( select, ownProps ) => {
	...
	const termFilters = Array.isArray( ownProps.termFilters ) ? ownProps.termFilters : _flatten( postTypeObjects.map( o => o ? o.taxonomies : [] ) );

	// Get all terms objects for the current term filters.
	const terms = termFilters.map( slug => getEntityRecord( 'root', 'taxonomy', slug ) )
		.filter( term => !! term && term.visibility.public );
